...
export interface NotebookPage {
  id: string;
  createdAt: number;
  backgroundType: 'blank' | 'lined' | 'grid';
  elements: NotebookElement[];
  drawings: DrawingPath[];
}

export type NotebookElement = NotebookTextBox | NotebookDropdown | NotebookDrawing | NotebookGroup;

export interface AbstractNotebookElement {
  id: string;
  type: 'text' | 'dropdown' | 'drawing' | 'group' | 'chart' | 'shape';
  x: number;
  y: number;
  width?: number;
  height?: number;
...
export interface NotebookTextBox extends AbstractNotebookElement {
  type: 'text';
  content: string;
  width: number;
  height: number;
...
export interface NotebookDropdown extends AbstractNotebookElement {
  type: 'dropdown';
  category: 'Material' | 'Formula' | 'Fragrance' | 'Inventory' | 'Equipment' | 'App';
...
export interface NotebookDrawing extends AbstractNotebookElement {
  type: 'drawing';
  points: { x: number; y: number }[];
  color: string;
  strokeWidth: number;
  width: number;
  height: number;
  drawingType?: 'freehand' | 'straight' | 'curve';
}

export interface NotebookGroup extends AbstractNotebookElement {
  type: 'group';
  width: number;
  height: number;
  items: NotebookElement[];
}
