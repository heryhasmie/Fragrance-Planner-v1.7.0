...
import { Beaker, BookOpen, Calculator, ListChecks, Timer, Database, DollarSign, MessageSquare, Download, Upload, X, Receipt, Wrench, Sun, Moon, Sparkles, Palette, Trash2, Package, Droplets, Users, TrendingUp, Settings as SettingsIcon, Book } from 'lucide-react';
import { useLocalStorage } from './hooks/useLocalStorage';
import { Formula, Fragrance, PlannedBatch, Maceration, RawMaterial, PriceEntry, Feedback, ShipmentOption, BudgetItem, BudgetPlan, Equipment, Theme, BottlingPlan, InventoryItem, AppSettings, Notebook } from './types';
...
export default function App() {
  ...
  const [notebooks, setNotebooks] = useLocalStorage<Notebook[]>('fragrance_notebooks', []);
  ...
  const confirmPurge = () => {
    ...
    if (purgeOptions.notebooks) setNotebooks([]);
    setPurgeModalOpen(false);
  };
  ...
  const confirmExport = () => {
    ...
    if (exportOptions.notebooks) data.notebooks = notebooks;
  ...
  const confirmImport = () => {
    ...
    if (importOptions.notebooks && importData.notebooks) {
      setNotebooks(mergeData(notebooks, importData.notebooks));
      summaryParts.push(`${importData.notebooks.length} Notebooks`);
    }
